...
colcon build --packages-select energia_sim
...
└── urdf/
    ├── energia_rover.urdf.xacro        # Base robot model
    └── energia_rover_gazebo.xacro      # Model + Gazebo plugins
```
...
```bash
ros2 launch energia_sim spawn_rover.launch.py x_pose:=5.0 y_pose:=2.0 z_pose:=0.2
```

### Modify Robot Properties

Edit `urdf/energia_rover.urdf.xacro`:
